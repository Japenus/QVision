...
QPixmap FormatTransfer::matToQpixmap(Mat &datatype)
{
    if(datatype.empty()) return QPixmap();
    Mat res;
    if (datatype.channels() == 1) {
        cvtColor(datatype, res, COLOR_GRAY2RGB);
    } else if (datatype.channels() == 3) {
        cvtColor(datatype, res, COLOR_BGR2RGB);
    } else if (datatype.channels() == 4) {
        res = datatype;
    } else {
        return QPixmap();
    }
